...
_Use_decl_annotations_ NTSTATUS DriverEntry(PDRIVER_OBJECT driver_object,
  ...
  // VVMM initilization must occur after logging is initialized.
  ...
  // FCD initilization must occur after logging is initialized.
...
_Use_decl_annotations_ static void DriverpDriverUnload(
  ...
  //
  // 1. We must invoke the breakpoint manager termination function first so
  //  that we can perform cleanup in VMX root mode.
  //
  ...
  //
  // 2. Devirtualize all processors and execute the remaining termination code.
  //
  VmTermination();

#ifdef CFG_ENABLE_DR_FACADE
  //
  // 3. We must invoke the facade termination function after all BPM-managed
  //  breakpoints have been cleared, and after we devirtualize all processors.
  //
