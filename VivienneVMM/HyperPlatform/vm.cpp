...
#pragma warning(suppress : 6014) // Memory leak.
  ...
  *reinterpret_cast<ProcessorData **>(vmm_stack_data) = processor_data;
  ...
  // Finish FCD initialization in VMX root mode, but before executing VMLAUNCH.
#pragma warning(suppress : 28122) // IRQL too low.
  FcdVmxInitialization();
#endif

  // Do virtualize the processor
#pragma warning(suppress : 28123) // IRQL too high.
  VmpLaunchVm();
...
_Use_decl_annotations_ static bool VmpSetupVmcs(
  ...
  VmxPinBasedControls vm_pinctl = {VmpAdjustControlValue(
      (use_true_msrs) ? Msr::kIa32VmxTruePinbasedCtls : Msr::kIa32VmxPinbasedCtls,
  ...
  VmxProcessorBasedControls vm_procctl = {VmpAdjustControlValue(
      (use_true_msrs) ? Msr::kIa32VmxTrueProcBasedCtls : Msr::kIa32VmxProcBasedCtls,
  ...
  vm_procctl2_requested.fields.enable_invpcid = false;          // for Win10
...
#ifdef CFG_ENABLE_EPT
  vm_procctl2_requested.fields.ept_violation_ve = true;
#else
...
_Use_decl_annotations_ static bool VmpIsHyperPlatformInstalled() {
  ...
  return cpu_info[0] == CFG_VVMM_SENTINEL;
}
